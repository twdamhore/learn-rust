# Changelog - Lesson 042: Traits - defining, implementing, default methods
...
- **Issues found**:
  ...
  - Exercise 4 is ambiguous about whether 'takes any Drawable' means trait bounds or trait objects (lesson 045)
- **Recommendations**:
  - Standardize exercise format
  - Clarify Exercise 4 to specify trait bounds (since trait objects are lesson 045)
...
- **Split needed**: No
- **Key issues**:
  - By this point learner has already used impl Trait syntax in lessons 033 and 038 - this lesson retroactively formalizes it
...
- **Issues**: Exercise 4 ambiguity -- "write a function that takes any Drawable" should explicitly specify trait bounds, not trait objects (lesson 045). Exercise format inconsistency.
...
- **Needs splitting**: No
- **Pacing context**: Retroactively formalizes trait syntax already used ad-hoc in lessons 033 and 038.
- **Unresolved from prior reviews**: (1) Should acknowledge learner already used impl Trait for Type in lessons 033/038. (2) Exercise format inconsistency (same as 039). (3) Exercise 4 ambiguous on trait bounds vs trait objects -- should specify trait bounds (trait objects are lesson 045). All unresolved since v4.
- **New findings**: None
...
- **Genuinely unresolved**: Exercise 4 "write a function that takes any Drawable" should specify to use trait bounds, not trait objects (lesson 045 topic)
...
- **Why**: Ambiguous wording could lead learner to lesson 045 content prematurely
...
- **Pacing**: Good
- **Issues**: None — excellent backward references to prior informal trait usage in lessons 033 and 038
- **Changes made**: Changelog updated only
