# Changelog - Lesson 097a: Capstone Project 2 - Part A: Axum Web API Basics
## Section 21: Capstone Projects

### v1 - Created from split (2026-03-03)
- Split from original lesson 097 which was rated OVERLOADED (~3-4 hrs)
...
- **New findings**: Minor -- add note explaining why Arc<Mutex<>> is needed for shared state, connecting back to lesson 060 (Shared state -- Mutex, RwLock, Arc<Mutex<>>)
...
- **Needs split**: No
- **Progression**: Excellent split — provides the missing axum introduction. Should note connection back to lesson 060 (Arc<Mutex<>>).
- **Changelog reconciliation**: All prior findings consistent
- **Genuinely unresolved**: None
- **Recommendation**: Add note connecting Arc<Mutex<>> usage back to lesson 060 when task content is next updated
...
- **Recommendations**: Add brief note connecting back to lesson 060
- **Changes made**: Changelog updated only
...
- **Estimated time**: 80-100min
- **Pacing**: Heavy
- **Issues**: Lesson 060 (shared state) not listed as prerequisite despite Arc<Mutex<Vec>> usage; no note connecting back to lesson 060
- **Changes made**: Added lesson 060 prerequisite and Arc<Mutex> note in Exercise 1, task file updated
