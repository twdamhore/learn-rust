...
## Prerequisites
- Lesson 065 (Shared state — `Mutex`, `Arc<Mutex<T>>`)
- Lesson 068-064 (async/await, Tokio basics)
- Lesson 097 (serde - for JSON serialization)
...
_Split from original lesson 112 which was rated OVERLOADED (~3-4 hr). This part uses in-memory storage only. SQLite and auth are in Part B (113). This provides the axum introduction that was missing from the curriculum._
