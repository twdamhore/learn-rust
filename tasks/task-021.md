# Lesson 021: Structs - named, tuple, unit structs
...
- [ ] **Exercise 3 - Rectangle**: Define a `Rectangle` struct with `width` and `height` as `f64` fields. Write a standalone function `area(rect: &Rectangle) -> f64` (methods come in lesson 022). Create several rectangles including one using struct update syntax and print their areas.
