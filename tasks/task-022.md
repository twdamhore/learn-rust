# Lesson 022: Methods and associated functions (impl blocks)
...
## Exercises
- [ ] **Exercise 1 - Rectangle methods**: Add an `impl` block to the `Rectangle` struct from lesson 021 with methods: `area(&self) -> f64`, `perimeter(&self) -> f64`, and `can_hold(&self, other: &Rectangle) -> bool` (returns true if `self` can fully contain `other`). Write tests for each method.
