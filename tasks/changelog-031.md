# Changelog - Lesson 031: Custom error types, From trait, error conversion
...
- **Issues found**:
  - **Prerequisite issue**: Requires implementing `From` trait before traits are formally taught (lesson 040)
...
- **Needs split**: No
- **Progression**: CRITICAL prerequisite gap -- requires implementing Display, Error, From traits but trait syntax not taught until lesson 040 (9 lessons away).
...
- **Unresolved from prior reviews**: CRITICAL (since v4): Requires impl Display/Error/From for Type -- trait impl syntax not taught until lesson 040 (9 lessons later). Must provide template code or a prerequisite mini-lesson explaining impl Trait for Type.
- **New findings**: None
...
- **Pacing**: Heavy
- **Issues**: Requires trait impl (lesson 040) but scaffolding template present.
...
- **Changes made**: Added source() return type explanation (`Option<&(dyn Error + 'static)>` with forward references to lessons 043/046) to Objective 3; marked Exercise 4 as STRETCH, task file updated
