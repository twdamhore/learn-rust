# Changelog - Lesson 020: Methods and associated functions (impl blocks)
...
- **Issues found**:
  - Exercise 3 introduces builder pattern (formally taught in lesson 079)
  - Exercise 3 uses `.into()` for string conversion which requires the `Into` trait (lesson 044)
- **Recommendations**:
...
- **Estimated time**: 65-85min
- **Pacing**: Medium-Heavy
- **Changes made**: Replaced `Config::default()` with `Config::new()` in Exercise 3 and added note that Default trait is in lesson 044
- **Why**: Config::default() requires Default trait (lesson 044) and trait impl syntax (lesson 040), both not yet taught
