# Changelog - Lesson 059: Rc<T>, Arc<T> - reference counting, shared ownership
...
- **Issues found**:
  - Exercise 2 requires RefCell (lesson 060 -- forward reference) to create reference cycle
  - Exercise 4 requires spawning threads (lesson 063 -- forward reference)
- **Recommendations**:
...
- **Split needed**: No
- **Key issues**:
  - Exercise 2 (reference cycle) uses RefCell not taught until lesson 060
  - Exercise 4 uses thread::spawn not taught until lesson 063
...
- **Issues**: Exercise 2 requires RefCell (lesson 060) -- provide boilerplate with "preview" note. Exercise 4 uses thread::spawn (lesson 063) -- add brief note. Both flagged since v4, unresolved.
...
- **Needs splitting**: No
- **Pacing context**: Second heavy lesson in cluster (058-059-060-061/062)
- **Unresolved from prior reviews**: (1) Exercise 2 uses RefCell (lesson 060) -- add preview note with boilerplate, (2) Exercise 4 uses thread::spawn (lesson 063) -- add preview note. Both flagged since v4.
- **New findings**: None
...
- **Why**: RefCell not taught until lesson 060; learner needs concrete pattern
