# Changelog - Lesson 054: Box<T> - heap allocation, recursive types
...
- **Changes noted**: BST exercise (insert + contains + in_order traversal) could take 20-30 min alone for recursive Rust types. Vec<Box<dyn Shape>> duplicates task 043. Deref/Drop overlap with lesson 045.
...
- **Key issues**:
  ...
  - Deref/Drop overlaps lesson 045
...
- **Pacing**: Heavy
- **Needs split**: No
- **Issues**: Exercise 2 (BST) too ambitious — insert+contains+in_order traversal. Reduce to insert+contains, make in_order optional. DUPLICATE EXERCISE — Exercise 4 (Vec<Box<dyn Shape>>) repeats pattern from task 043. Replace with different trait. Start of heavy cluster (054-055-056-057a/b). Flagged since v4.
...
- **Pacing**: Heavy
- **Issues**: Ex 4 (Box<dyn Animal>) nearly identical to lesson 043 trait object pattern — should use different domain. UNRESOLVED from v4.
