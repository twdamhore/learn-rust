# Changelog - Lesson 073: The Future Trait and Cancellation
## Section 14: Async Rust

### v1 - Created from split (2026-03-03)
- Split from original lesson 073 which was rated OVERLOADED (~2-3 hrs)
- **Reason for split**: Lesson 073 combined four major hard topics (Future trait impl, manual poll loop, Pin/Unpin, cancellation) -- likely the hardest single lesson in the entire curriculum. For a learner with no C/C++ background, this was unrealistic under 2 hours
...
- **Pacing**: Heavy
- **Issues**: Timeout combinator exercise uses Pin<Box<F>> and self: Pin<&mut Self> but Pin is not taught until 074. Consider reordering 073/074 or marking Timeout as STRETCH. 4 exercises is heavy — Countdown + TimerFuture + Cancellation alone are 70-80 min
- **Changes made**: Changelog updated only
...
- **Estimated time**: 80-100min
- **Pacing**: Heavy
- **Issues**: Marked Exercise 3 (Timeout combinator) as [STRETCH] — uses Pin<Box<F>> and Pin<&mut Self> which are not taught until lesson 074
- **Changes made**: Task file updated — Exercise 3 header changed to [STRETCH] with note to attempt after completing 074
