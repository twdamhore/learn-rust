# Lesson 050a: Advanced Lifetimes - Part A: Higher-Ranked Trait Bounds
...
## Added
- Split from original lesson 050 (v6 pacing review)
...
> **Closure crash course** (formally covered in lesson 051): Closures are Rust's anonymous functions, written as `|arg| expression` or `|arg| { body }`. They capture variables from their environment. Rust has three closure traits:
...
>
> For this lesson, you just need to know the syntax `|x| x + 1` and that `Fn(&str) -> &str` means "a closure that borrows a `&str` and returns a `&str`". Full coverage comes in lesson 051.
