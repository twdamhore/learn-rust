# Changelog - Lesson 080: Attribute macros, real-world macro patterns
...
- **Issues found**:
  - Assumes strong comfort with proc macro crates from lesson 078
  - Exercise 2 (#[validate_positive]) requires parsing function signatures -- harder than it sounds
- **Recommendations**:
  - If lesson 078 is scaled back, ensure learner has enough proc macro experience before this lesson

### v5 - Cross-curriculum review (2026-03-03)
- Full 100-lesson review for pacing realism, prerequisite ordering, and progressive difficulty
- **Pacing**: Heavy (~1.5 hr) for 1-2 hour target
- **Changes noted**: Depends heavily on lesson 078 comfort with proc macro setup. Exercise 3 (validate_positive) requires parsing function signatures -- harder than it appears due to syn API complexity. If 071 is reduced, 072 needs to accommodate.
- **Why**: Dependency chain on potentially-reduced lesson 078.
...
- **Split needed**: No
- **Key issues**:
  - Depends on lesson 078 being addressed (now split into 078/079)
  - validate_positive attribute requires syn API for function signatures (harder than it looks)
- **Action taken**: No split required. With lesson 078 now split into 078/079, learner will have proper proc macro foundation. validate_positive difficulty remains a concern -- consider providing syn parsing hints when lesson is started
...
- **Pacing**: Heavy
- **Needs split**: No
- **Issues**: Exercise 3 (#[validate_positive]) requires parsing syn::ItemFn inputs — syn API more complex than it appears. Parsing hints/skeleton needed. Three heavy in a row (070, 079, 072). Unresolved since v4.
...
- **Needs splitting**: No
- **Pacing context**: Third heavy lesson in a row after 070 and 079; learner fatigue risk
...
- **Needs split**: No
- **Progression**: Third heavy lesson in a row (070, 079, 072) — fatigue risk
...
- Added syn::ItemFn parsing scaffold (starter code) to Exercise 3 (#[validate_positive]) in task-080.md
...
- Reviewed. **Time**: 80-95min. **Pacing**: Heavy. **Issues**: Fourth heavy lesson in 070-072 run, mitigated by 079 STRETCH deferral. **Changes**: Changelog only.
