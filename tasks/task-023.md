# Lesson 023: Pattern matching - match, if let, while let, destructuring
...
## Exercises
- [ ] **Exercise 1 - Exhaustive enum matching**: Using the `Message` enum from lesson 022, write a `match` expression that handles every variant and returns a descriptive `String`. Try removing one arm and observe the compiler error. Add a `catch-all` arm with `_` and discuss when catch-alls are appropriate vs dangerous.
