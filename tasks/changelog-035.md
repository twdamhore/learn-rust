# Changelog - Lesson 035: Testing with ownership, Result, and custom types
...
- **Issues found**:
  - Exercise 4 uses generic-looking syntax `Result<T,E>` before generics taught (lesson 041)
- **Recommendations**:
...
- **Issues**: Exercise 4 uses generic Result<T,E> in helper function signature -- generics not taught until lesson 041. Should use concrete types instead (flagged since v4, unresolved).
...
- **Unresolved from prior reviews**: Exercise 4 uses generic Result<T,E> syntax before generics taught (lesson 041). Replace with concrete types. Flagged since v4.
- **New findings**: None
...
- **Pacing**: Good
- **Issues**: Exercise 4 should explicitly reference lesson 033 Exercise 4's Config/ConfigError types for reuse
- **Changes made**: Changelog updated only
...
- **Changes made**: Reframed Exercise 1 to clarify compile-time ownership enforcement (not runtime test); added lesson 033 cross-reference tip to Exercise 4, task file updated
