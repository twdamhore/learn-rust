# Lesson 026: Crates, packages, use, re-exports, prelude pattern
...
- [ ] **Exercise 2 - Re-exports for clean API**: Create a library with internal modules `internal::parser` and `internal::formatter`. In `lib.rs`, use `pub use internal::parser::parse;` and `pub use internal::formatter::format_output;` so users of the crate only need `use mycrate::{parse, format_output}` rather than knowing the internal structure. Write a binary crate that depends on this library and uses the re-exported API. To make a binary crate depend on a local library, add a path dependency to Cargo.toml: `mylib = { path = "../mylib" }`. Workspace setup (an alternative approach) is covered in lesson 027.
