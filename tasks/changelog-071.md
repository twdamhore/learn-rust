# Changelog - Lesson 071: Async I/O Part B: Async Networking
## Section 14: Async Rust

### v1 - Created from split (2026-03-03)
- Split from original lesson 070 which was rated HEAVY (~2+ hrs)
- **Reason for split**: Original lesson 070 combined async file I/O, TCP networking, timeouts, and a concurrent file processor into 4 exercises. The TCP echo server alone is a substantial mini-project (binding, accepting, spawning per connection, line-based read/write, testing). Networking deserves its own focused lesson to build the server properly and extend it into something more interesting (chat or protocol).
