# Changelog - Lesson 036: HashMap, HashSet, BTreeMap, entry API
...
- **Issues found**:
  ...
  - Exercise 3 (Cache<K,V>) uses generic type parameters before generics taught (lesson 039)
- **Recommendations**:
...
- **Issues**: Exercise 3 Cache<K,V> uses generic type parameters before lesson 039 -- should use concrete types like HashMap<String, String> (flagged since v4, unresolved). Entry API (3 distinct patterns) deserves focused attention.
...
- **Unresolved from prior reviews**: (1) Exercise 3 Cache<K,V> uses generics before lesson 039 -- replace with concrete types (e.g., HashMap<String, String>). (2) Three consecutive heavy lessons (036-037-038) flagged for fatigue. Both unresolved since v4.
- **New findings**: None
...
- **Changelog reconciliation**: "Exercise 3 uses generic Cache<K,V>" flagged since v4 as unresolved -- but task file already uses concrete `String` types with note about generics in lesson 039. Resolved.
...
- **Pacing**: Heavy
- **Issues**: Exercise 3 used `impl FnOnce() -> String` closure parameter, which is a forward reference to closures (lesson 051)
- **Changes made**: Simplified Exercise 3 from `get_or_insert_with` with `impl FnOnce() -> String` to `get_or_insert` taking a `String` value directly. Added note that closure-based version is covered after lesson 051.
