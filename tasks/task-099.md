# Lesson 099: Clap - CLI argument parsing, subcommands, derive API
...
## Prerequisites
- Lesson 097 (serde) — Exercise 2 uses serde/JSON for task persistence.
...
- [ ] **Exercise 2 -- Subcommands**: Create a `task-manager` CLI with subcommands: `add <TITLE> [--priority <low|medium|high>]` and `list [--filter <STATUS>]`. Use `#[derive(Subcommand)]` for the enum. Store tasks in a JSON file using serde (connecting to lesson 097). Implement both subcommands end-to-end.
